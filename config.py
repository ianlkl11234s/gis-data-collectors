...
# 落後超過 CATCHUP_LAG_HOURS 時，單輪放寬到 CATCHUP_FILES_PER_STREAM 檔以追回進度
GLOBAL_EVENTS_CATCHUP_FILES_PER_STREAM = int(os.getenv('GLOBAL_EVENTS_CATCHUP_FILES_PER_STREAM', '24'))
GLOBAL_EVENTS_CATCHUP_LAG_HOURS = float(os.getenv('GLOBAL_EVENTS_CATCHUP_LAG_HOURS', '6'))
# master index 真實缺洞：單輪最多跳過幾個 slot（避免大洞讓單輪掃太久）
GLOBAL_EVENTS_MAX_SKIP_SLOTS = int(os.getenv('GLOBAL_EVENTS_MAX_SKIP_SLOTS', '96'))
# 已進 index 但超過此時數仍抓不到 ZIP → 視為缺洞跳過，不再永久卡住 cursor
GLOBAL_EVENTS_ARTIFACT_STALE_HOURS = float(os.getenv('GLOBAL_EVENTS_ARTIFACT_STALE_HOURS', '6'))
# 待評估佇列保鮮期；過期候選只記 receipt 不再送 LLM（新鮮度優先）
GLOBAL_EVENTS_PENDING_TTL_HOURS = int(os.getenv('GLOBAL_EVENTS_PENDING_TTL_HOURS', '48'))
# 單輪 Stage1 送件軟預算（秒），留餘裕給 COLLECT_TIMEOUT=1800
GLOBAL_EVENTS_STAGE1_BUDGET_SECONDS = int(os.getenv('GLOBAL_EVENTS_STAGE1_BUDGET_SECONDS', '1200'))
...
# OpenRouter models fallback（逗號分隔）；留空＝行為與請求 body 完全不變
GLOBAL_EVENTS_QWEN_FALLBACK_MODELS = os.getenv('GLOBAL_EVENTS_QWEN_FALLBACK_MODELS', '')
GLOBAL_EVENTS_QWEN_TIMEOUT = int(os.getenv('GLOBAL_EVENTS_QWEN_TIMEOUT', '90'))
GLOBAL_EVENTS_QWEN_MAX_CANDIDATES = int(os.getenv('GLOBAL_EVENTS_QWEN_MAX_CANDIDATES', '100'))
GLOBAL_EVENTS_QWEN_CHUNK_SIZE = int(os.getenv('GLOBAL_EVENTS_QWEN_CHUNK_SIZE', '10'))
GLOBAL_EVENTS_QWEN_MAX_OUTPUT_TOKENS = int(os.getenv('GLOBAL_EVENTS_QWEN_MAX_OUTPUT_TOKENS', '8192'))
# Post-hoc provider-cost alert, not a hard pre-request spend ceiling.
