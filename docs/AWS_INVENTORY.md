...
| `deploy-assets/global-maritime/gfw-hourly/` | GFW unified derived releases（AIS grid/tracks + SAR unmatched） | v2 first release `2026-08-20`; v3 remains shadow-only | 待下次完整 inventory | **successful derived releases are retained; no raw GFW response is stored.** Reader root only indexes current + previous release for rollback and is 60s/SWR300; immutable release objects are retained even after leaving that bounded index. Zeabur daily 08:30 Asia/Taipei; Cloudflare root-edge cache remains unverified. |
...
| `ripe_atlas_internet_health/` | ripe_atlas_internet_health | 62,888 bytes create-only smoke object | 1 verified smoke；daily inventory 待更新 | **deployment 6a954164... RUNNING；migration 383 gates 全綠。production one-shot 9d6d032c... succeeded 913/56/0，keys 1001/2001 與 SHA-256 readback 通過；scheduled f97ce10e... succeeded 916/56/0，5min enabled。raw 僅供 private S3 復原，public RPC 0 rows** |
| `ripe_ris_live/raw/v1/` | ripe_ris_live | bounded smoke 2 gzip（2,033 / 112 records）＋ production automatic 1 gzip（1,292 records） | 完整 inventory 待更新 | **2026-08-31 bounded 18min smoke：1 startup partial、2 succeeded，2 gzip + manifest 全量 GET/SHA-256/readback 通過。Zeabur Settings Replicas=1、Overview 1/1，production flags true/gate=1；截至 15:05 UTC 正式排程 3 runs（partial 1 / succeeded 2），成功窗各 6 unknown observations，current 6/6 fresh、public 0；production automatic archive 1 個亦通過 full GET/SHA/manifest。無 distributed lease、internal-only** |
