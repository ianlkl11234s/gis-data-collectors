...
| `isohe_port_marine` | ISOHE 商港 Wave/Current/Tide/Wind | 同上 VM（部署前需 Taiwan egress smoke） | [`external/isohe_port_marine_vm/`](../external/isohe_port_marine_vm/) |
...
另外 **`external/vm_common/vm_buffer.py` 是所有 VM collector 共用的必帶檔**（本地 buffer +
連線 retry，DB 掛掉時該輪資料存本地、下輪自動補寫）。collector 內用
`from vm_buffer import ...` import，部署時跟主檔放同一個 app dir 即可。

### Step 4：部署到 VM
- 共用同一台 HiCloud VM（資源充足前都堆同台、降低成本）
- App dir 分開：`/opt/<name>/`
- **記得一起 scp `external/vm_common/vm_buffer.py` 到 app dir**（與 collector 主檔同目錄）
...
### 本地 buffer（DB outage 防丟資料，2026-07-02 加）
所有 VM collector 都掛 `vm_buffer.py`：DB 寫入失敗 → 該輪資料存
`<DATA_DIR>/buffer/*.json` → 下輪 cron 開頭自動補寫（成功才刪檔）。
保留 3 天（cdc 週跑型 30 天）、單目錄上限 200 檔，超過丟最舊。

驗證指令（VM 上）：
```bash
ls -lt /var/lib/ship-ais/data/buffer/          # 有積壓檔 = DB 曾寫入失敗
grep -E "buffer (補寫|存)" /var/log/ship-ais/collect.log | tail   # 補寫紀錄
```
buffer 目錄持續累積不消化 = DB 一直連不上，去看 Supabase 狀態。
...
重點：scp 6 個檔（+ 共用的 `vm_common/vm_buffer.py`）→ 跑 setup_vm.sh → 填 .env → 跑 setup_cron.sh。**不需要 git clone、不需要 GitHub 認證**。
