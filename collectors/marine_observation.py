...
def isohe_payload_to_long(port_code: str, kind: str, payload: Any, collected_at: str) -> tuple[list[dict], list[dict]]:
    ...
    for row in rows:
        ...
        if not observed_at:
            continue
        lon = number_or_none(row.get("Station_Longitude") or row.get("Longitude") or row.get("lon"))
        lat = number_or_none(row.get("Station_Latitude") or row.get("Latitude") or row.get("lat"))
